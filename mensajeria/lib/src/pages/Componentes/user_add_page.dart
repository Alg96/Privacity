...
class _HomePageState extends State<UserAdd> {
  final _session = Session();
  String canal, correo, parsed;

  List data = [];

  List<NoMiembro> _notes = List<NoMiembro>();
  List<NoMiembro> _notesForDisplay = List<NoMiembro>();

  Future<List<NoMiembro>> fetchNotes() async {
    canal = widget.name;

    var url = '${AppConfig.apiHost}/getNoMiembros.php';

    var response = await http.post(url, body: {"canal": canal});

    var notes = List<NoMiembro>();

    if (response.statusCode == 200) {
      var notesJson = json.decode(response.body);
      for (var noteJson in notesJson) {
        notes.add(NoMiembro.fromJson(noteJson));
      }
    }
    return notes;
  }
