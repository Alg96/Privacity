...
import 'package:mensajeria/src/pages/ChatScreen/chatscreen_channel_user.dart';
...
class _HomePageState extends State<ChannelGroup> {
  String nombre;
  ...
  Future<List<Channel>> fetchNotesChannel() async {
    ...
    var url = '${AppConfig.apiHost}/getChaIn.php';
  ...
  Future<List<Group>> fetchNotesGroup() async {
    ...
    var url = '${AppConfig.apiHost}/getGroupIn.php';
            ...
            onTap: () {
              widget.tipo == 'Channel'
                  ? Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => ChatChannelUser(
                              _notesForDisplayChannel[index].nombre,
                              _notesForDisplayChannel[index].enlace,
                              _notesForDisplayChannel[index].idchat,
                              _notesForDisplayChannel[index].propietario,
                              "1")))
                  : widget.tipo == 'Group'
                      ? Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => ChatGroup(
                                  ...
                                  _notesForDisplayGroup[index].descripcion,
                                  "1",
                                  _notesForDisplayGroup[index].propietario)))
                      : CircularProgressIndicator(
                          backgroundColor: Colors.blue,
                          strokeWidth: 10.0,
                        );
